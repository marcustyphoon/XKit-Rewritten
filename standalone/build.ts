...
const linksSection = `
      <section id="links">
        <ul>
          ${
            process.env.GIST_ID
              ? `<li><a href="https://gist.githubusercontent.com/marcustyphoon/${process.env.GIST_ID}" target="_blank">Installation</a></li>`
              : ''
          }
...
if (process.env.ALWAYS_ENABLED) {
  replacements.push(
    ...[
      {
        from: /(?<=<input type="checkbox" class="toggle-button" aria-label="Enable this feature")/g,
...
fs.writeFileSync('src/source-url.txt', sourceLink);
