...
const unzipped = unzipSync(contents);
const sourceLink = decoder.decode(unzipped['source-url.txt']);
...
if (fs.existsSync(`${gitDir}/about.md`)) {
  const about = fs.readFileSync(`${gitDir}/about.md`, 'utf8');
  fs.writeFileSync(
    `${gitDir}/about.md`,
    about.replace(
      /Source for this program is available at.*/,
      `Source for this program is available at ${sourceLink}`
    ),
    { flag: 'w+' }
  );
