...
const createGenericNotificationReplyData = async (notificationProps) => {
  ...
  // Generic quote content fallback code. We cannot find the post to produce a reblog, so unconditionally make a new post.
  ...
  try {
    ...
    if (canReblog !== false && !blog?.isPasswordProtected) {
      return mode === 'ask'
          ...
          showModal({
            ...
            buttons: [
              ...
              dom('button', { class: 'blue' }, {
                click () {
                  hideModal();
                  resolve({ parentPostId, parentTumblelogUUID, reblogKey });
                },
              }, ['Reblog']),
              dom('button', { class: 'blue' }, {
                click () {
        ...
        : { parentPostId, parentTumblelogUUID, reblogKey };
    }
  } catch {}
  return new Promise(resolve =>
    showModal({
      title: 'Quote Replies',
      message: ['The target post cannot be reblogged, but you can reply in a new post.'],
      buttons: [
        modalCancelButton,
        dom('button', { class: 'blue' }, {
          click () {
...
const createReplyData = async ({ type, replyingBlogName, replyingBlogUuid, targetPostSummary, targetPostUrl, replyContent, targetTumblelogName, targetPostId }) => {
  const reblogData = await getReblogData({ targetTumblelogName, targetPostId });
  if (reblogData) {
    ...
    return {
      content,
      tags,
      parent_post_id: reblogData.parentPostId,
      parent_tumblelog_uuid: reblogData.parentTumblelogUUID,
      reblog_key: reblogData.reblogKey,
    };
  }
  const verbiage = {
