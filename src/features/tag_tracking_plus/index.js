...
/**
 * @typedef {object} StoredUnreadCount
 ...
 * @property {boolean} loaded Used to keep a visually consistent sidebar load sequence between from-storage and from-api-fetch initial count loads. If `false`, this stored value is not used to render the sidebar, as it would not have been fetched yet.
 */

const storedUnreadCountsStorageKey = '_caches.tag_tracking_plus.storedUnreadCounts';
/** @type {Record<string, StoredUnreadCount>} */
...
const storedCountIsStale = (tag, ttl) =>
...
const loadStoredCount = async tag => {
  ...
  storedUnreadCounts[tag].loaded = true;
  await browser.storage.local.set({ [storedUnreadCountsStorageKey]: storedUnreadCounts });
};

const updateSidebar = () => {
  const countData = trackedTags.map(tag => ({ tag, ...storedUnreadCounts[tag] ?? {} }));
  countData
    .filter(({ loaded }) => loaded)
    .forEach(({ tag, unreadCountString }) => {
  ...
  sidebarItem.dataset.loading = countData.some(({ loaded }) => !loaded);
  sidebarItem.dataset.hasNew = countData.some(({ loaded, unreadCountString }) =>
    loaded && unreadCountString !== '0',
  );
};
...
const refreshNextCount = async () => {
  ...
  if (nonLoadedTag) {
    storedUnreadCounts[nonLoadedTag]
    ...
    const oldestTag = [...trackedTags]
      ...
      .at(0);
    if (storedCountIsStale(oldestTag, PER_TAG_BACKGROUND_REFRESH_INTERVAL)) {
...
export const main = async function () {
  ...
  for (const tag of Object.keys(storedUnreadCounts)) {
    if (storedCountIsStale(tag, INITIAL_LOAD_STORED_COUNT_MAX_AGE)) {
      delete storedUnreadCounts[tag];
    }
  }
  // Mark (fresh) tracked stored counts as not yet loaded, to keep a visually consistent sidebar load sequence between from-storage and from-api-fetch initial count loads.
  for (const tag of Object.keys(storedUnreadCounts)) {
