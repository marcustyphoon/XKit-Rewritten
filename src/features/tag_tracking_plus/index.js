...
const updateSidebar = () => {
  const countData = trackedTags.map(tag => ({ tag, ...unreadCounts[tag] ?? {} }));
  countData
...
export const onStorageChanged = async (changes) => {
  const {
    ...
    [unreadCountsStorageKey]: storedUnreadCountsChanges,
  ...
  if (storedUnreadCountsChanges) {
    unreadCounts = storedUnreadCountsChanges.newValue;
    updateSidebar();
  }
