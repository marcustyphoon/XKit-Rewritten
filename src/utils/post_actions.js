 ...
 * @param {string} options.iconUrl Icon url to use, such as one produced by import.meta.resolve or browser.runtime.getURL
 ...
 */
export const registerPostOption = async function ({ id, iconUrl, onclick, showInAskForm = false }) {
  postOptions[id] = {
    element: label({ class: 'xkit-post-option', [displayBlockUnlessDisabledAttr]: '' }, [
      button({ click: onclick }, [buildSvg(iconUrl)]),
    ]),
    showInAskForm,
  };
